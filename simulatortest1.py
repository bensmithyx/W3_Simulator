...
class Timer:
    def __init__(self, name, time, text,xxis, yxis, state):
        self.starttime = time
        self.name = name
        self.time = time
        self.timetext = ''
    ...
    def display(self):
        screen.blit(font.render(f'{self.text}: {self.timetext}', True, (0, 0, 0)), (self.xxis, self.yxis))
...
class Emergency:
    ...
    def start_event(self):
        ...
        #messages user that the current selected astronaut is in danger
        for astro in astronauts:
            if astronauts[active_astronaut] and inside_pod(mx,my,astro.rect.centerx,astro.rect.centery,radius) == True:
                screen.blit(font2.render('ASTRONAUTS ARE IN DANGER', False, (0, 0, 0)), (100, 900))
            else:
                screen.blit(font2.render('ASTRONAUTS ARE SAFE', False, (0, 0, 0)), (100, 950))
...
class Astronaut(pygame.sprite.Sprite):
    ...
    def check_alive(self):
        if self.health <= 0:
            ...
            screen.blit(font2.render('THIS ASTRONAUT HAS DIED', True, (0, 0, 0)), (100, 850))
...
clocks = [Timer('acms_20delay', 6, 'delay',1500, 150, False),
          Timer('doors', 5, 'doors', 1500, 200, False),
          Timer('fire', 30, 'FIRE', 1500, 250, False),
          Timer('bio', 30, 'BIOHAZARD', 1500, 300, False),
          Timer('radiation', 30, 'RADIATION', 1500, 350, False), 
          Timer('airquality', 30, 'AIRQUALITY', 1500, 400, False),
          Timer('airpressure', 30, 'AIRPRESSURE', 1500, 450, False),
          
          Timer('airlockrefill',10,'Airlock (Empty/Refill)',1500 ,550,False),
          Timer('Evacuation',259200,'Evacuation',1500 ,600,False),

          # Timers for Event Detected Messages
          Timer('fire', 30, 'DISASTER DECTECTED!' , 1500, 150, False),
          Timer('bio', 30, 'DISASTER DECTECTED!   ' , 1500, 150, False),
          Timer('radiation', 30, 'DISASTER DECTECTED!      ' , 1500, 150, False),
          Timer('airquality', 30, 'DISASTER DECTECTED!         ' , 1500, 150, False),
          Timer('airpressure', 30, 'DISASTER DECTECTED!            ' , 1500, 150, False),

          #timers for unlocking gates
          Timer('fire', 35, 'UNLOCKING GATES (ALL CLEAR) IN' , 1500,  500, False),
          Timer('bio', 35, 'UNLOCKING GATES (ALL CLEAR) IN   ' , 1500,  500, False),
          Timer('radiation', 35, 'UNLOCKING GATES (ALL CLEAR) IN      ' , 1500,  500, False),
          Timer('airquality', 35, 'UNLOCKING GATES (ALL CLEAR) IN         ' , 1500,  500, False),
          Timer('airpressure', 35, 'UNLOCKING GATES (ALL CLEAR) IN            ' , 1500,  500, False),

          #timers for Extra information   
          Timer('fire', 30, 'EXTRA INFO FOR EMERGENCYS' , 1500,  750, False),
          Timer('bio', 30, 'EXTRA INFO FOR EMERGENCYS   ' , 1500,  750, False),
          Timer('radiation', 30, 'EXTRA INFO FOR EMERGENCYS      ' , 1500,  750, False),
          Timer('airquality', 30, 'EXTRA INFO FOR EMERGENCYS         ' , 1500,  750, False),
          Timer('airpressure', 30, 'EXTRA INFO FOR EMERGENCYS            ' , 1500,  750, False),
          Timer('fire', 30, 'FFS PUMPS SUPPRESSANT GAS IN' , 1500,  800, False),
          Timer('bio', 30, 'RMS REMOVES BIOHAZARD IN' , 1500,  850, False),
          Timer('radiation', 30, 'RMS REMOVES RADIATION IN' , 1500,  900, False),
          Timer('airquality', 30, 'AQMS CORRECTS AIRQUALITY IN' , 1500,  950, False),
          Timer('airpressure', 30, 'APMS CORRECTS AIRPRESSURE IN' , 1500,  1000, False),
          ]
